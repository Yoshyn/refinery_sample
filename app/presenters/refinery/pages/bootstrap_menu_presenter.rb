...
module Refinery
  module Pages
    class BootstrapMenuPresenter < MenuPresenter
      ...
      def render_menu(items)
        content_tag(menu_tag, :id => dom_id, :class => css, :role => :navigation) do
          content_tag(:div, :class => 'container-fluid') do
            ...
            buffer << content_tag(:div, :class => 'navbar-header') do
              link_to Refinery::Core.site_name, context.refinery.root_path, :class => 'navbar-brand'
            end

            buffer << content_tag(:div, :class => 'collapse navbar-collapse') do
      ...
      def render_menu_items(menu_items, tag_css: 'nav navbar-nav navbar-right', role: '')
